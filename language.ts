...
function shuffled<T>(list: (T[] | string)): (T[] | string) {
    var newlist = [];
...
function choose<T>(list: string, exponent?: number): string
function choose<T>(list: T[], exponent?: number): T
function choose<T>(list: T[] | string, exponent: number = 1): T | string {
    return list[Math.floor(Math.pow(random(), exponent) * list.length)];
}
...
export function makeName(lang, key) {
    ...
    while (true) {
        ...
        if (random() < 0.5) {
            ...
            var w1 = capitalize(getWord(lang, random() < 0.6 ? key : ''));
            var w2 = capitalize(getWord(lang, random() < 0.6 ? key : ''));
            if (w1 == w2) continue;
            if (random() > 0.5) {
...
function makeBasicLanguage(): Language {
    return {
        phonemes: {
            C: "ptkmnls",
            V: "aeiou",
